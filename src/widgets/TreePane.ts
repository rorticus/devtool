...
export interface TreePaneProperties extends ThemedProperties {
	...
	/**
	 * Toggle only when the arrow is clicked, versus the whole row
	 */
	toggleOnArrowClick?: boolean;
...
export interface RowProperties extends ThemedProperties {
	 ...
	 * @param isContent A flag that determines if the out content was clicked (the "arrow") or the label.  Returns `true`
	 *                  for the content and `false` if the label.
	 */
	onClick?(key: string | number | undefined, isContent: boolean): void;
...
export class Row extends ThemedBase<RowProperties> {
	private _onclick(evt: MouseEvent) {
		this.properties.onClick && this.properties.onClick(this.properties.key, this.meta(Matches).get('content', evt));
	}
	private _ondblclick() {
...
export default class TreePane extends ThemedBase<TreePaneProperties> {
	...
	private _onRowClick(key: string, isContent: boolean) {
		...
		this.properties.selected !== key && onItemSelect && onItemSelect(key);
		const item = this._findItem(key);
		if (!item) {
			throw new Error(`Uncached TreePane row ID: "${key}"`);
		}
		if (item.children && onItemToggle && (!toggleOnArrowClick || isContent)) {
			onItemToggle(key);
		}
	}
